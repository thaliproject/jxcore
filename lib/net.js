...
Server.prototype._listen2 = function(address, port, addressType, backlog, fd) {
  ...
  // The port_features trick, and the corresponding logic that it activates
  // in libuv, forces sockets to be reused across instances for any given port.
  // Presumably this is intended to make tcp/http server code work with
  // JXcore multithreading without any modification. However it makes strong
  // assumptions on how such user code is written and in fact if a socket is
  // explicitly closed, the next time we listen on the same port we end up
  // listening on a bad file descriptor (EBADF).
  // For the above reasons, we disable this logic when we are on a restartable
  // instance.
  var _port = -1;
